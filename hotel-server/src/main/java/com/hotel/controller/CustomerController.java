...
public class CustomerController {
  ...
  @ApiOperation("查询客人信息")
  public ResponseEntity<ResultDto> findCustomers(
      @ApiParam("客人姓名") @RequestParam(value = "name", required = false) String name,
  ...
  public ResponseEntity<ResultDto> updateCustomer(
      @Validated(UpdateCustomerGroup.class) @RequestBody CustomerDto customerDto) {
    // 判断客人是否存在
    ...
    return new ResponseEntity<>(
        ResultDto.builder()
            ...
            .data(
                Converters.customer2Dto(
                    customerService.update(Converters.dto2Customer(customerDto))))
            .build(),
        HttpStatus.OK);
  }
  ...
  private boolean isDuplicated(List<Customer> customerByIdCard, CustomerDto customerDto) {
    return (customerByIdCard != null && customerByIdCard.size() > 1)
        || (customerByIdCard.size() == 1
            && !customerByIdCard.get(0).getId().equals(customerDto.getId()));
  }
