...
import com.hotel.entity.Income;
...
public class RoomController {
  ...
  @ApiOperation("搜索客房")
  @ApiImplicitParams({
    @ApiImplicitParam(name = "roomNo", dataType = "string", value = "客房号", paramType = "form"),
  ...
  public ResponseEntity<ResultDto> create(
      @RequestBody @Validated(AddRoomGroup.class) RoomDto roomDto) {
  ...
  public ResponseEntity<ResultDto> login(
      @RequestBody @Validated(LoginGroup.class) RoomDto roomDto) {
    ...
    for (CustomerDto customerDto : customerDtos) {
      List<Customer> customerByIdCard =
          customerService.findCustomerByIdCard(customerDto.getIdCard());
      if (customerByIdCard.size() > 1) {
        return new ResponseEntity<>(
            ResultDto.builder()
                .success(false)
                .code(RetCode.RETCODE_40001)
                .message(customerDto.getIdCard() + "身份证存在多个客人信息")
                .build(),
            HttpStatus.BAD_REQUEST);
      }
            ...
            if (room.getRoomNo().equals(roomDto.getRoomNo())) {
              return new ResponseEntity<>(
                  ResultDto.builder()
                      ...
                      .message(customer.getIdCard() + "身份证号的客人已登记过" + room.getRoomNo() + "客房:")
                      .build(),
                  HttpStatus.BAD_REQUEST);
            }
          }
    ...
    if (rooms == null || rooms.size() == 0) {
      return new ResponseEntity<>(
          ResultDto.builder()
              ...
              .message(roomDto.getRoomNo() + "号客房不存在")
              .build(),
          HttpStatus.BAD_REQUEST);
    }
    ...
    if (rooms.size() > 1) {
      return new ResponseEntity<>(
          ResultDto.builder()
              ...
              .message(roomDto.getRoomNo() + "号客房存在多个房间")
              .build(),
          HttpStatus.BAD_REQUEST);
    }
    ...
    if (room.getStatus() == RoomStatus.UNUSED) {
      return new ResponseEntity<>(
          ResultDto.builder()
              ...
              .message(roomDto.getRoomNo() + "号客房为不可订状态")
              .build(),
          HttpStatus.BAD_REQUEST);
    }
    ...
    if (beds == null || beds.size() == 0) {
      return new ResponseEntity<>(
          ResultDto.builder()
              ...
              .message(roomDto.getRoomNo() + "号客房没有维护床铺信息，不可登记")
              .build(),
          HttpStatus.BAD_REQUEST);
    }
    ...
    if (currentMoney == 0) {
      return new ResponseEntity<>(
          ResultDto.builder()
              ...
              .message(roomDto.getRoomNo() + "号客房没有维护当前房价或房价为0，不可登记")
              .build(),
          HttpStatus.BAD_REQUEST);
    }
  ...
  public ResponseEntity<ResultDto> updateRoom(@RequestBody RoomDto roomDto) {
    ...
    RoomDto roomDto1 =
        Converters.room2RoomDto(roomService.updateRoom(Converters.roomDto2Room(roomDto)));
    return new ResponseEntity<>(
        ResultDto.builder()
            .success(true)
  ...
  public ResponseEntity<ResultDto> getRoom(
      @ApiParam(value = "客房ID", required = true) @PathVariable("id") Integer id) {
  ...
  private int calculateInterval(RoomType type, Date in, Date out) {
    ...
    switch (type) {
      ...
      case MONTH:
        result =
            (int) Math.ceil((out.getTime() - in.getTime()) / 30.0f / 24.0f / 3600.0f / 1000.0f);
        break;
      default:
